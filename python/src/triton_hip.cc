...
void init_triton_driver(py::module &&m) {
  ...
  // cuda device
  py::class_<drv::hip_device, drv::device>(m, "hip_device")
      .def(py::init([](int dev_id, bool take_ownership) {
        hipDevice_t handle;
        drv::dispatch::hipGetDevice(&handle, dev_id);
        return new drv::hip_device(handle, take_ownership);
      }));
  // host device
  ...
  py::class_<drv::host_stream, drv::stream>(m, "host_stream")
      .def(py::init<>());
  // cuda stream
  py::class_<drv::hip_stream, drv::stream>(m, "hip_stream")
      // py doesn't support opaque pointer (e.g., hipStream_t) so
      // we assume it has been converted to uint64_t
      .def(py::init([](uint64_t handle, bool take_ownership) {
        return std::unique_ptr<drv::hip_stream>(new drv::hip_stream((hipStream_t)handle, take_ownership));
      }))
      .def("enqueue", [](drv::hip_stream *self, drv::kernel *kernel,
  ...
  py::class_<drv::hip_module, drv::module>(m, "hip_module")
      .def("ptx", &drv::hip_module::ptx)
      .def("cubin", [](drv::hip_module *self) { return py::bytes(self->cubin()); })
      .def("llir", &drv::hip_module::llir);
